...
export default function BillRequests() {
  ...
  const getTypeColor = (type: string) => {
    return type === 'clientRequest' ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary';
  };
  
  // Filter and sort the data
  const filteredData = billRequestsData.filter(request => {
    const matchesSearch = 
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
          ...
          <div className="flex gap-2">
            ...
            <div className="relative inline-block">
              ...
              <UserCircle className="absolute right-2.5 top-2.5 size-4 pointer-events-none text-muted-foreground" />
            </div>
          </div>
        </div>
      ...
      <div className="rounded-lg border bg-white dark:bg-gray-800 shadow-sm overflow-hidden"
          ...
          <table className="w-full caption-bottom text-sm">
            ...
            <tbody className="divide-y">
              {currentItems.map((request) => (
                <tr key={request.id} className="border-b transition-colors hover:bg-gray-50 dark:hover:bg-gray-900/20">
                  ...
                  <td className="px-4 py-3 align-middle">
                    <div className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getTypeColor(request.type)}`}>
                  ...
                  <td className="px-4 py-3 align-middle">
                    <div className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusColor(request.status)} text-white`}>
        ...
        <div className="border-t p-4 flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredData.length)} of {filteredData.length} requests
          </div>
