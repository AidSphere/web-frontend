...
const medicinesWithPrices: Record<string, MedicineWithPrice[]> = {
  "BR-7842": [
    { id: 1, name: "Amoxicillin 500mg", category: "Antibiotics", quantity: 60, unit: "Tablets", status: "Available", price: 24.99, selected: true, notes: "" },
    { id: 2, name: "Lisinopril 10mg", category: "Cardiovascular", quantity: 30, unit: "Tablets", status: "Available", price: 12.50, selected: true, notes: "" },
    { id: 3, name: "Metformin 1000mg", category: "Diabetes", quantity: 60, unit: "Tablets", status: "Low Stock", price: 18.75, selected: true, notes: "Limited stock available. Consider alternative dosage." },
    { id: 4, name: "Atorvastatin 20mg", category: "Cardiovascular", quantity: 30, unit: "Tablets", status: "Available", price: 22.99, selected: true, notes: "" },
    { id: 5, name: "Albuterol Inhaler", category: "Respiratory", quantity: 2, unit: "Inhalers", status: "Out of Stock", price: 45.00, selected: false, notes: "Currently out of stock. Expected to be available in 2 weeks." },
    { id: 6, name: "Omeprazole 20mg", category: "Gastrointestinal", quantity: 30, unit: "Capsules", status: "Available", price: 15.25, selected: true, notes: "" },
    { id: 7, name: "Levothyroxine 50mcg", category: "Thyroid", quantity: 30, unit: "Tablets", status: "Available", price: 10.50, selected: true, notes: "" },
    { id: 8, name: "Hydrochlorothiazide 25mg", category: "Cardiovascular", quantity: 30, unit: "Tablets", status: "Available", price: 8.99, selected: true, notes: "" },
    { id: 9, name: "Sertraline 50mg", category: "Mental Health", quantity: 30, unit: "Tablets", status: "Available", price: 14.75, selected: true, notes: "" },
    { id: 10, name: "Ibuprofen 800mg", category: "Pain Relief", quantity: 30, unit: "Tablets", status: "Available", price: 7.50, selected: true, notes: "" },
    { id: 11, name: "Fluticasone Nasal Spray", category: "Allergy", quantity: 1, unit: "Bottle", status: "Available", price: 28.99, selected: true, notes: "" },
    { id: 12, name: "Vitamin D3 2000IU", category: "Supplements", quantity: 90, unit: "Tablets", status: "Available", price: 12.25, selected: true, notes: "" },
  ]
};

// Additional medicines that could be added to the quotation
const availableMedicines: MedicineWithPrice[] = [
  { id: 101, name: "Amlodipine 5mg", category: "Cardiovascular", quantity: 30, unit: "Tablets", status: "Available", price: 11.50, selected: false, notes: "" },
  { id: 102, name: "Losartan 50mg", category: "Cardiovascular", quantity: 30, unit: "Tablets", status: "Available", price: 16.75, selected: false, notes: "" },
  { id: 103, name: "Gabapentin 300mg", category: "Neurology", quantity: 60, unit: "Capsules", status: "Available", price: 22.99, selected: false, notes: "" },
  { id: 104, name: "Escitalopram 10mg", category: "Mental Health", quantity: 30, unit: "Tablets", status: "Available", price: 18.25, selected: false, notes: "" },
  { id: 105, name: "Montelukast 10mg", category: "Respiratory", quantity: 30, unit: "Tablets", status: "Available", price: 24.50, selected: false, notes: "" }
];
...
export default function PrepareQuotationPage() {
  ...
  const [additionalItems, setAdditionalItems] = useState<MedicineWithPrice[]>(availableMedicines);
  ...
  const categories: string[] = ['All', ...Array.from(
    new Set([...quotationItems, ...additionalItems].map(med => med.category))
  )];
  ...
  const handleAddMedicine = (medicine: MedicineWithPrice): void => {
    ...
    if (!quotationItems.some(item => item.id === medicine.id)) {
      ...
      setAdditionalItems(prev => prev.filter(item => item.id !== medicine.id));
    }
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Prepare Quotation</h1>
        ...
        <div className="flex flex-wrap gap-2">
          <button 
            ...
            className="inline-flex items-center justify-center whitespace-nowrap rounded-md bg-white dark:bg-gray-800 border px-4 py-2 text-sm font-medium text-gray-900 dark:text-gray-100 shadow-sm transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
            style={{ borderRadius: 'var(--radius)' }}
          >
          ...
          <button 
            ...
            className="inline-flex items-center justify-center whitespace-nowrap rounded-md bg-primary text-white px-4 py-2 text-sm font-medium shadow-sm transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
            style={{ borderRadius: 'var(--radius)' }}
          >
      ...
      <div className="grid gap-6 md:grid-cols-3">
                      ...
                      <tr key={medicine.id} className="border-b transition-colors hover:bg-gray-50 dark:hover:bg-gray-900/20">
                        ...
                        <td className="px-4 py-3 align-middle">
                          <div className="flex items-center justify-center">
                            ...
                            <input 
                              ...
                              step="0.01"
                              min="0"
                              className="w-16 px-2 py-1 text-right border rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                              style={{ borderRadius: 'var(--radius)' }}
                            />
                          </div>
                        </td>
                        <td className="px-4 py-3 align-middle">
                          <span 
                            className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${
                              medicine.status === 'Available' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                              medicine.status === 'Low Stock' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                        ...
                        <td className="px-4 py-3 align-middle">
                          <div className="flex items-center justify-center gap-2">
                            <button 
                              onClick={() => {
                                const notes = prompt('Enter notes for this item:', medicine.notes);
                                if (notes !== null) {
              ...
              <div className="p-4 border-t">
                ...
                <ul className="space-y-2">
                  {quotationItems
                    ...
                    .map(item => (
                      <li key={`note-${item.id}`} className="flex items-start gap-2">
            ...
            {showAddMedicines && (
              <div className="border-t">
                <div className="p-4 border-b">
                  <h4 className="font-medium mb-3">Add Additional Medicines</h4>
          ...
          <div className="rounded-lg border bg-white dark:bg-gray-800 shadow-sm overflow-hidden"
            ...
            <div className="p-4">
              <textarea
                ...
                className="w-full min-h-24 p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                style={{ borderRadius: 'var(--radius)' }}
              ></textarea>
            </div>
          </div>
        ...
        <div className="space-y-6">
          {/* Quotation Summary */}
          <div className="rounded-lg border bg-white dark:bg-gray-800 shadow-sm sticky top-4"
            style={{ borderRadius: 'var(--radius)' }}>
            <div className="border-b px-4 py-3">
            ...
            <div className="p-4">
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="w-20">
                      <input
                        ...
                        className="w-full px-2 py-1 text-right border rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                        style={{ borderRadius: 'var(--radius)' }}
                      />
                    </div>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label htmlFor="validity" className="text-muted-foreground">Validity (days):</label>
                    <div className="w-20">
                      <input
                        id="validity"
                        type="number"
                        ...
                        max="90"
                        value={validityDays}
                        onChange={(e) => setValidityDays(Math.min(90, Math.max(1, parseInt(e.target.value) || 30)))}
                        className="w-full px-2 py-1 text-right border rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
                        style={{ borderRadius: 'var(--radius)' }}
                      />
                    </div>
                ...
                <div className="flex items-center bg-primary/5 border border-primary/20 rounded-md p-3 text-sm">
                  <InfoIcon className="size-4 text-primary mr-2 shrink-0" />
                  <p>This quotation will be valid until {new Date(Date.now() + validityDays * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4">
              <ul className="space-y-3 text-sm">
                <li className="flex items-start gap-2">
                  ...
                  <span>You can add notes to medicines that are low in stock or unavailable.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="size-4 text-green-500 shrink-0 mt-0.5" />
                  <span>Add alternative medicines from the &quot;Add Medicines&quot; section.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="size-4 text-green-500 shrink-0 mt-0.5" />
                  <span>Set expiration date for the quotation to encourage timely decisions.</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="size-4 text-green-500 shrink-0 mt-0.5" />
                  <span>Save as draft if you need to finalize details later.</span>
                </li>
              </ul>
            </div>
          </div>
